...
## ⚠️注意事项
1. **根目录只出现client、server、filesystem和.gitignore、readme**，**每个人只在自己的目录下更新**，不然会导致混乱。  
2. client与server不在一台机器，使用**ssh通信**；server与filesystem在一台机器，使用**websocket通信**，因此目录尽量独立。
3. 交流很重要！各个部分与相邻的部分对接，需要对接什么接口或者协议就在群里发消息交流。
4. 下面的部分可以不随时更新。那样太麻烦。  
## 1. 目录结构说明
```
.
